...
enum Format {
    ...
    /// Formats a Celsius value in the supplied unit. Callers should observe
    /// `AppSettings.temperatureUnitKey` via `@AppStorage` so their bodies
    /// re-render when the user changes the unit.
    static func temp(_ celsius: Double, in unit: TemperatureUnit) -> String {
        String(format: "%.1f %@", unit.convert(fromCelsius: celsius), unit.symbol)
    }
